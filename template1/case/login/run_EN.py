...
import time
import sys
sys.path.append(r'D:\template\template1\common')
...
def run_case(all_case, report_path):
    ...
    runner = HTMLTestRunnerEN.HTMLTestRunner(stream=fp,
                                           title=u'auto run result ：',
                                           description=u'runner details：')
...
if __name__ == "__main__":
    # 测试用例的路径、匹配规则
    case_path = "D:\\template\\template1\\case\\login"
    rule = "test*.py"
    all_case = add_case(case_path, rule)   # 加载用例
    # 生成测试报告的路径
    report_path = "D:\\template\\template1\\report"
